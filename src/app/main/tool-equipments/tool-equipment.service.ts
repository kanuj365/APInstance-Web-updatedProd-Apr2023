...
export class ToolEquipmentService {
    ...
    getToolEquipments(): Observable<any> {
        return this.http.HttpGet(this.http.Services.ToolEquipment.GetAll).pipe(
    ...
    GetAllByCriteria(filters: any): Observable<any> {
        return this.http
            .HttpPost(
                this.http.Services.ToolEquipment.GetAllByCriteria,
                filters
            )
            .pipe(
                retry(this.http.Services.RetryServieNo),
    ...
    deleteToolEquipmentById(toolEquipmentId: string) {
        ...
        return this.http
            .HttpPost(
                this.http.Services.ToolEquipment.Delete,
                toolEquipmentParams
            )
            .pipe(
                retry(this.http.Services.RetryServieNo),
    ...
    getVTPByUser(userModel: UserModel): Observable<any[]> {
        let academicYearRequest = this.http.GetMasterDataByType(
            {
                DataType: "AcademicYears",
                UserId: userModel.UserTypeId,
                SelectTitle: "Academic Year",
            },
            false
        );
        let vtpRequest = this.http.GetMasterDataByType({
            DataType: "VocationalTrainingProvidersByUserId",
            RoleId: userModel.RoleCode,
            UserId: userModel.UserTypeId,
            SelectTitle: "Vocational Training Provider",
        });
        // let vtpRequest = this.commonService.GetVTPByAYId(userModel.RoleCode, userModel.UserTypeId, userModel.AcademicYearId);
        let schoolRequest = this.http.GetMasterDataByType(
            {
                DataType: "SchoolsByVT",
                ParentId: userModel.UserTypeId,
                SelectTitle: "School",
            },
            false
        );
        let divisionRequest = this.http.GetMasterDataByType({
            DataType: "Divisions",
            RoleId: userModel.RoleCode,
            UserId: userModel.UserTypeId,
            ParentId: userModel.DefaultStateId,
            SelectTitle: "Division",
        });
    ...
    getAcademicYearSectorByUser(vtId: any): Observable<any[]> {
        let academicYearRequest = this.http.GetMasterDataByType({
            DataType: "AcademicYearsByVT",
            RoleId: "VT",
            ParentId: vtId,
            SelectTitle: "Academic Year",
        });
        let sectorRequest = this.http.GetMasterDataByType({
    ...
    getJobRoleToolListRawMaterialBySector(
        sectorParams: any
    ): Observable<any[]> {
        let jobRoleRequest = this.http.GetMasterDataByType({
            DataType: "TEJobRolesForVT",
            RoleId: sectorParams.AcademicYearId,
            UserId: sectorParams.VTId,
            ParentId: sectorParams.SectorId,
    ...
    initToolsAndEquipmentsData(userModel: UserModel): Observable<any[]> {
        ...
        let vtRequest = this.http.GetMasterDataByType(
            {
                DataType: "VocationalTrainersByVC",
                RoleId: userModel.RoleCode,
                ParentId: userModel.UserTypeId,
        ...
        let vcRequest = this.http.GetMasterDataByType(
            {
        ...
        let vtpRequest = this.commonService.GetVTPByAYId(
            userModel.RoleCode,
            userModel.UserTypeId,
            userModel.AcademicYearId
        );
